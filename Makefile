...
fmt:              ## Format code using black & isort.
	$(ENV_PREFIX)isort vigilant_winner/
	$(ENV_PREFIX)black -l 79 vigilant_winner/
	$(ENV_PREFIX)black -l 79 tests/
	$(ENV_PREFIX)black -l 79 .github/templates/

.PHONY: lint
lint:             ## Run pep8, black, mypy linters.
	$(ENV_PREFIX)flake8 vigilant_winner/
	$(ENV_PREFIX)flake8 .github/templates/
	$(ENV_PREFIX)black -l 79 --check vigilant_winner/
	$(ENV_PREFIX)black -l 79 --check tests/
	$(ENV_PREFIX)black -l 79 --check .github/templates/
	$(ENV_PREFIX)mypy --ignore-missing-imports vigilant_winner/

.PHONY: test
test: lint        ## Run tests and generate coverage report.
	$(ENV_PREFIX)pytest -v --cov-config .coveragerc --cov=vigilant_winner -l --tb=short --maxfail=1 tests/
...
release:          ## Create a new tag for release.
	...
	@git tag $${TAG}
	@echo "$${TAG}" > vigilant_winner/VERSION
	@$(ENV_PREFIX)gitchangelog > HISTORY.md
	@git add vigilant_winner/VERSION HISTORY.md
...
switch-to-poetry: ## Switch to poetry package manager.
	...
	@rm -rf .venv
	@poetry init --name=vigilant_winner --author=Torugo
