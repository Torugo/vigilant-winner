...
# Move module files
rm -rf vigilant_winner
rm -rf tests
cp -R .github/templates/flask/module vigilant_winner
...
then
    ...
    echo "then run vigilant_winner or python -m vigilant_winner"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run vigilant_winner or python -m vigilant_winner or poetry run vigilant_winner"
fi
